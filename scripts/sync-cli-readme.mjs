...
const destPath = path.join(repoRoot, 'packages', 'cli', 'README.md');
...
function transform(content) {
  ...
  content = content
    ...
    .replace(/\]\(packages\/cli\/src\/discord\/README\.md\)/g, `](${GH_BLOB_HEAD}/packages/cli/src/discord/README.md)`)
    .replace(/\]\(packages\/cli\/src\/telegram\/README\.md\)/g, `](${GH_BLOB_HEAD}/packages/cli/src/telegram/README.md)`)
...
function main() {
  ...
  console.log(`Synced CLI README from ${path.relative(repoRoot, srcPath)} -> ${path.relative(repoRoot, destPath)}`);
}
