...
} from '../packages/core/src/llm/registry/provider-runtime.js';
...
type DextoSupportedFileType = 'pdf' | 'image' | 'audio' | 'video' | 'document';
...
function modelToGeneratedModel(
    ...
    return {
        ...
        ...(typeof model.temperature === 'boolean'
            ...
            : {}),
        ...(typeof model.tool_call === 'boolean' ? { supportsToolCall: model.tool_call } : {}),
...
async function syncLlmRegistry() {
    ...
    for (const providerId of providerIds) {
        ...
        const runtime = inferProviderRuntimeMetadata({
            providerId,
            npm: p.npm,
            api: p.api,
        });
    ...
    const modelsDevMetadataByProvider: Partial<Record<string, GeneratedProviderMetadata>> = {};
    for (const providerId of providerIds) {
        const metadata = getProviderMetadata(modelsDevJson[providerId], providerId);
        if (!metadata) continue;
        modelsDevMetadataByProvider[providerId] = metadata;
    }
...
`;
    ...
    const providersBody = `
import type { ProviderRuntimeMetadata } from './provider-runtime.js';
