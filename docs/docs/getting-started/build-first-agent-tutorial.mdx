...
When you provide a prompt directly, Dexto automatically uses CLI mode for quick, one-shot execution.
...
For interactive conversations and a richer experience, run without a prompt:

```bash
dexto --agent market-research-assistant.yml
```

This opens the Web UI (the default mode), providing a web interface for multi-turn conversations with your agent.
