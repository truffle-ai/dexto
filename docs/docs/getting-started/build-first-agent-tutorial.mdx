...
Your agent should now respond like a pirate in your terminal. When you provide a prompt directly (via `-p` or as a positional argument), Dexto automatically uses CLI mode for quick, one-shot execution.

:::tip Interactive Mode
For interactive conversations, use `dexto --agent agent.yml` without a prompt. This opens the Web UI by default, or you can use `dexto --agent agent.yml --mode cli` for terminal-based interaction.

Short-hand: `dexto -a <agent>` works the same as `dexto --agent <agent>`
:::
...
Start an interactive session with your enhanced agent in the Web UI:

```bash
dexto --agent agent.yml
```

This will open the Dexto Web UI (the default mode), which provides a web interface for talking to your agents.
