...
Use the `agents list` command to list the in-built agents.
...
```bash
dexto agents install music-agent
```
...
```bash
dexto agents install music-agent --no-inject-preferences
```
...
Now let's rerun the `agents list` command to see the installed agents.
...
- ✅ List available agents in dexto with the `dexto agents list` command
- ✅ Install agents with the `dexto agents install` command
- ✅ Uninstall agents with the `dexto agents uninstall` command
- ✅ Use the `--no-inject-preferences` argument with `dexto agents install` command to use default LLM preferences
