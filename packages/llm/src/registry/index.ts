...
function unknownModel(provider: LLMProvider, model: string): LlmCatalogError {
    return new LlmCatalogError(
        'MODEL_UNKNOWN',
...
interface EffectiveMaxInputTokensConfig {
...
export const LLM_REGISTRY: Record<LLMProvider, ProviderInfo> = {
    ...
    'dexto-nova': {
        models: MODELS_BY_PROVIDER['dexto-nova'],
        baseURLSupport: 'optional',
        supportedFileTypes: GENERIC_UPLOAD_FILE_TYPES,
        supportsCustomModels: true,
...
function cloneProviderInfo(provider: ProviderInfo): ProviderInfo {
    return {
        ...provider,
        models: provider.models.map(cloneModelInfo),
        supportedFileTypes: [...provider.supportedFileTypes],
        ...(provider.modelsDev
            ? {
                  modelsDev: {
                      ...provider.modelsDev,
                      env: [...provider.modelsDev.env],
                  },
              }
            : {}),
    };
...
export function getProvider(provider: LLMProvider): ProviderInfo;
export function getProvider(provider: string): ProviderInfo | null;
export function getProvider(provider: string): ProviderInfo | null {
    if (!isKnownProvider(provider)) return null;
    return cloneProviderInfo(LLM_REGISTRY[provider]);
}

export function listModels(provider?: LLMProvider): ModelLookup[] {
    const providers = provider === undefined ? LLM_PROVIDERS : [provider];
    return providers.flatMap((providerId) =>
        getAllModelsForProvider(providerId).map((model) => ({
            providerId,
            modelId: model.name,
...
export function getModel(provider: LLMProvider, model: string): ModelInfo | null {
    const modelInfo = findModelInfo(provider, model);
    return modelInfo === null ? null : cloneModelInfo(modelInfo);
}

export function resolveGatewayOrigin(provider: LLMProvider, model: string): GatewayOrigin | null {
    ...
    const providerByOpenRouterPrefix = new Map<string, LLMProvider>(
        Object.entries(OPENROUTER_PREFIX_BY_PROVIDER).map(([nativeProvider, prefix]) => [
            prefix,
            nativeProvider as LLMProvider,
        ])
    );
    const providerId = providerByOpenRouterPrefix.get(providerPrefix);
    return providerId === undefined ? null : { providerId, modelId };
}
...
export function getEffectiveMaxInputTokens(
    ...
    try {
        ...
        if (error instanceof LlmCatalogError && error.code === 'MODEL_UNKNOWN') {
            ...
            throw unknownModel(config.provider, config.model);
        } else {
