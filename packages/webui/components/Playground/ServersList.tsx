      ...
      {servers.length === 0 && !isLoading && !error && (
        <div className="flex-1 flex items-center justify-center p-4">
          <div className="text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">Connect a server to get started</p>
          </div>
        </div>
      )}
          ...
          {filteredServers.map((server) => (
            ...
            >
              <div className="flex items-center justify-between gap-2">
                ...
                <Badge
                  variant="secondary"
                  className={cn('text-xs px-1.5 py-0 h-5 flex items-center gap-1', getStatusColor(server.status))}
                >
