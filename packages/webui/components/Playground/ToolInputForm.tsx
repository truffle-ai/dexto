  ...
  const renderInput = (key: string, prop: JsonSchemaProperty) => {
    ...
    if (prop.type === 'object' || prop.type === 'array') {
      return (
        <Textarea
          ...
          onChange={(e: ChangeEvent<HTMLTextAreaElement>) => onInputChange(key, e.target.value, prop.type)}
          rows={5}
    ...
    return (
      <Input
        ...
        onChange={(e: ChangeEvent<HTMLInputElement>) => onInputChange(key, e.target.value, prop.type)}
        ...
        Object.entries(tool.inputSchema!.properties!).map(([key, prop]) => {
          ...
          return (
            <div key={key} className="grid gap-1.5">
              <div
                className={`flex ${
                  prop.type === 'boolean' ? 'flex-row items-center space-x-3' : 'flex-col'
                }`}
              >
                <Label
                  htmlFor={key}
      ...
      <div className="flex flex-wrap gap-2 pt-2">
        ...
        {hasInputs && Object.keys(inputs).length > 0 && (
          <>
            ...
            {onShareConfig && (
              <Button type="button" variant="outline" size="sm" onClick={onShareConfig}>
