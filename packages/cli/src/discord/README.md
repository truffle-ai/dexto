...
*   **Message Handling:** The bot connects to Discord using the Discord Gateway (WebSocket) and processes messages as they arrive based on `discord.js` library events. It does not use explicit short polling for messages.
...
*   **Tool Call Notifications:** The bot subscribes to `llm:tool-call` events on its `agentEventBus`. When a tool is about to be executed by the LLM service, a notification message (⚙️ Calling tool...) is sent to the Discord channel where the command originated.
