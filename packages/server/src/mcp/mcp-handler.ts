...
export async function initializeMcpServer(
    ...
    noOpLogger.info(`Registered MCP tool: '${toolName}'`);
...
export function createMcpHttpHandlers(mcpTransport: Transport) {
    ...
    const handlePost = async (req: IncomingMessage, res: ServerResponse, body: unknown) => {
        noOpLogger.info(`MCP POST /mcp received request body: ${JSON.stringify(body)}`);
        try {
    ...
    const handleGet = async (req: IncomingMessage, res: ServerResponse) => {
        noOpLogger.info('MCP GET /mcp received request, attempting to establish SSE connection.');
        try {
            await mcpTransport.handleRequest(req, res);
        } catch (err) {
            noOpLogger.error(`MCP GET error: ${JSON.stringify(err, null, 2)}`);
        }
    };
