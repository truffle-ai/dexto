...
/**
 * Input schema for delegate_to_url tool
 */
...
/**
 * Simple A2A client for basic delegation
 * Implements A2A Protocol v0.3.0 JSON-RPC transport
 */
class SimpleA2AClient {
    ...
    async sendMessage(message: string, sessionId?: string): Promise<any> {
        ...
        // Determine endpoint (try JSON-RPC first, fallback to legacy)
        const endpoints = [
            `${this.url}/v1/jsonrpc`, // A2A v0.3.0 JSON-RPC endpoint
            `${this.url}/jsonrpc`, // Alternative path
        ];
        ...
        for (const endpoint of endpoints) {
            try {
                ...
                // Return raw data if not standard JSON-RPC
                return data;
            } catch (error) {
    ...
    private extractTaskResponse(task: any): string {
        // Extract from history (get last agent message)
        ...
        // Fallback: return stringified task
    ...
    return {
        ...
        execute: async (input: unknown, _context?: ToolExecutionContext) => {
            ...
            try {
                ...
                // Use provided sessionId or generate new one for conversation tracking
                ...
                // Return formatted response with sessionId for conversation resumption
                return {
                    success: true,
                    agentUrl: url,
                ...
                // Re-throw DextoRuntimeError as-is
