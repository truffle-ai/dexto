...
export class ContextManager<TMessage = unknown> {
    ...
    /**
     * Removes the inline summary marker (isSummary) from a message.
     *
     * This is used during session-native compaction to preserve the full history
     * in the old session. After creating a continuation session with the summary,
     * the old session's summary marker is removed so filterCompacted() doesn't
     * hide the original messages.
     *
     * The summary message itself is preserved for historical record, but without
     * the isSummary marker it won't trigger read-time filtering.
     *
     * @returns true if a summary marker was removed, false if no summary found
     */
    async removeInlineSummaryMarker(): Promise<boolean> {
        const history = await this.historyProvider.getHistory();

        // Find the message with isSummary marker
        const summaryMessage = history.find((msg) => msg.metadata?.isSummary === true);

        if (!summaryMessage) {
            this.logger.debug(
                `removeInlineSummaryMarker: No isSummary marker found in session ${this.sessionId}`
            );
            return false;
        }

        // Update the message to remove the isSummary marker
        // Keep the message content for historical record, just remove the filtering marker
        const updatedMetadata = { ...summaryMessage.metadata };
        delete updatedMetadata.isSummary;
        // Mark it as having been processed for session-native compaction
        updatedMetadata.wasInlineSummary = true;
        updatedMetadata.convertedToSessionNative = Date.now();
        ...
        await this.historyProvider.updateMessage(updatedMessage);
        await this.historyProvider.flush(); // Ensure the update is persisted

        this.logger.debug(
            `removeInlineSummaryMarker: Removed isSummary marker from message ${summaryMessage.id} in session ${this.sessionId}`
        );
