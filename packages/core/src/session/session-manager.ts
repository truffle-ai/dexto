...
/**
 * Session type - flexible string to support custom types
 */
...
/**
 * Extensible session metadata with known fields
 */
export interface SessionMetadataFields {
    /** Session type for categorization */
    type: SessionType;
    /** Sub-agent specific data (only for sub-agent sessions) */
 ...
 * All metadata (including type) is consolidated in the metadata field for consistency.
 */
...
export class SessionManager {
    ...
    /**
     * Helper to normalize session type
     */
    private normalizeType(type?: string): SessionType {
        const normalized = type?.trim();
        return normalized || SessionManager.DEFAULT_SESSION_TYPE;
    }

    /**
     * Build session metadata from options
     */
    private async buildSessionMetadata(options?: {
        type?: string;
        subAgent?: Partial<SubAgentMetadata>;
        metadata?: Record<string, any>;
    }): Promise<SessionMetadataFields> {
        ...
        const type = this.normalizeType(requestedType);
        const baseMetadata: SessionMetadataFields = {
            type,
            ...(options?.metadata || {}),
        };
        ...
        if (wantsSubAgent) {
            ...
            baseMetadata.type = 'sub-agent';
            baseMetadata.subAgent = {
                parentSessionId,
            };
        }
    ...
    public async createSession(
        ...
        // Build consolidated metadata with type
        const metadata = await this.buildSessionMetadata(options);
    ...
    private async createSessionInternal(
        ...
        // Create new session data with consolidated metadata
        ...
        try {
            ...
            this.logger.info(`Created new session: ${id} [${options.metadata.type}]`);
            return session;
        } catch (error) {
    ...
    public async listSessions(filters?: {
        ...
        for (const key of sessionKeys) {
            ...
            const type = this.normalizeType(sessionData.metadata?.type);
    ...
    public async getSessionTitle(sessionId: string): Promise<string | undefined> {
        ...
        const sessionData = await this.services.storageManager
            .getDatabase()
            .get<SessionData>(sessionKey);
        return sessionData?.metadata?.title as string | undefined;
    }
