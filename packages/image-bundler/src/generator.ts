 ...
 * - Compatibility `imageMetadata` export (temporary; used by current CLI enrichment)
 */
...
function generateMetadata(definition: ImageDefinition, coreVersion: string): string {
    ...
    // Current CLI enrichment still reads `imageMetadata.bundledPlugins`. Long-term, image defaults
    // and resolver-provided plugins/tools should replace this pattern.
    if (definition.bundledPlugins && definition.bundledPlugins.length > 0) {
        metadata.bundledPlugins = definition.bundledPlugins;
    }

    return `/**
 * Compatibility metadata for current CLI enrichment.
 *
 * Note: This is NOT the image module contract. The authoritative metadata lives in
 * the default-exported DextoImageModule at \`image.metadata\`.
 */
...
/**
 * Compatibility metadata (used by current CLI enrichment)
 */
