...
**Direction update (2026-02-25):** [`UPDATED_DIRECTION.md`](./UPDATED_DIRECTION.md) captures verified upstream observations (OpenRouter catalog completeness, no release-date filtering in OpenCode) and the updated direction (pi-style multi-source registry + transport mapping).
...
- **Provider IDs are now models.dev-driven** in core (`packages/core/src/llm/providers.generated.ts` → `LLM_PROVIDERS` via `packages/core/src/llm/types.ts`), with a small Dexto overlay (`dexto-nova`, `openai-compatible`, `litellm`, `glama`, `local`, `ollama`).
  - **Backward compatibility is explicitly not required** for legacy provider IDs (`glm`, `vertex`, `bedrock`) or legacy auth storage.
- **Runtime scalability gap remains:** model/provider catalog is now dynamic, but model creation + reasoning behavior are still mostly implemented with provider-ID branches (`packages/core/src/llm/services/factory.ts`, `packages/core/src/llm/executor/provider-options.ts`, `packages/core/src/llm/reasoning/profile.ts`).
  - We need transport/API-kind routing (`provider.npm`-driven) plus explicit exception tables for known edge cases.
- **Reasoning alignment:** the reasoning PR introduced provider-specific edge handling that works for current paths; next step is to keep those semantics while moving to transport-kind routing so new providers inherit behavior without per-provider branching sprawl.
...
- Generated provider union from models.dev + overlay: `packages/core/src/llm/providers.generated.ts`, `packages/core/src/llm/types.ts`
...
- Runtime model creation is still mostly provider-ID switch logic: `packages/core/src/llm/services/factory.ts`
- Reasoning/provider options are still mostly provider-ID keyed: `packages/core/src/llm/executor/provider-options.ts`, `packages/core/src/llm/reasoning/profile.ts`
...
| `amazon-bedrock` | (API key/creds) | (N/A) | AWS chain in factory; bearer token is recognized in status helpers (`packages/agent-management/src/utils/api-key-store.ts`) | Canonical provider ID is models.dev’s `amazon-bedrock`; keep `/connect` guidance + explicit method choices |
| `google-vertex` | ADC | (N/A) | ADC-only (`GOOGLE_VERTEX_PROJECT`, etc.) | Canonical provider ID is models.dev’s `google-vertex`; keep `/connect` guidance + status surface |
...
### 7.5.1 Transport/API-kind routing (to avoid provider-ID spaghetti)

To scale reasoning + runtime behavior across 97+ models.dev providers without exploding switch-cases:
- Add a small internal `TransportKind`/`ApiKind` resolver:
  - Input: `providerId`, `modelId`, models.dev metadata (`provider.npm`, optional model-level `provider.npm` / `provider.api`).
  - Output: transport kind + SDK providerOptions key (`openai`, `anthropic`, `google`, `bedrock`, `openrouter`, etc.).
- Route both:
  - model creation (`factory.ts`)
  - reasoning/provider options (`provider-options.ts`, `reasoning/profile.ts`)
  through this resolver.
- Keep quirks in explicit exception tables (not scattered branches):
  - OpenRouter family gates (e.g. DeepSeek/GLM/Kimi exclusions),
  - Bedrock anthropic-vs-nova reasoning shape,
  - OpenAI/Codex-specific runtime rewrites.
- Reference patterns:
  - OpenCode providerOptions key mapping + remap: `~/Projects/external/opencode/packages/opencode/src/provider/transform.ts`
  - pi-mono `KnownApi` split and generation mapping: `~/Projects/external/pi-mono/packages/ai/src/types.ts`, `~/Projects/external/pi-mono/packages/ai/scripts/generate-models.ts`
...
- `scripts/sync-llm-models.ts` now emits both:
  - models snapshot (`packages/core/src/llm/registry/models.generated.ts`)
  - provider snapshot (`packages/core/src/llm/providers.generated.ts`, includes `env`/`npm`/`api`/`doc`)
- Remaining work is not snapshot generation; it is:
  - gateway catalog ingestion (OpenRouter + Vercel AI Gateway),
  - transport/API-kind runtime wiring,
  - `/connect` provider list expansion driven by the generated provider snapshot.
...
- [x] **0.3 Define provider auth method + profile types**
  - Deliverables:
...
- [x] **1.5.1 Generate a provider snapshot from models.dev (offline, committed)**
  - Deliverables:
  ...
  - Status notes (implemented):
    - `scripts/sync-llm-models.ts` generates `packages/core/src/llm/providers.generated.ts`.
    - `packages/core/src/llm/types.ts` consumes generated `LLM_PROVIDERS`.
    - Runtime provider wiring still needs transport/API-kind refactor (Phase 2.2/2.3.1).
...
- [ ] **2.3.1 Migrate reasoning/runtime branching to transport/API-kind + add regression suite**
  - Deliverables:
    - Introduce a single resolver that maps provider/model metadata to:
      - transport kind/api kind,
      - SDK providerOptions key namespace.
    - Refactor `provider-options.ts` + `reasoning/profile.ts` + `factory.ts` to use this resolver instead of broad provider-ID branching.
    - Preserve existing reasoning quirks through explicit exception tables (OpenRouter family allow/deny, Bedrock Anthropic-vs-Nova, provider-specific defaults).
    - Add regression tests that lock behavior before/after refactor:
      - transport mapping coverage test over all generated providers (`providers.generated.ts`) with explicit unsupported buckets,
      - reasoning variant/profile tests per transport with representative models,
      - provider-options output tests per transport + gateway model families,
      - schema validation tests for unsupported reasoning variants/budgets.
  - Exit:
    - New providers that share a transport inherit reasoning/runtime behavior without new provider-ID branches.
    - Existing reasoning tests still pass, and new mapping/behavior tests prevent regressions.
