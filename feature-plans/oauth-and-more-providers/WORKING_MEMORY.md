...
**Task:** **2.3.1 Transport/API-kind refactor for reasoning/runtime + regression suite**
**Status:** _Planned next_
**Branch:** `oauth-provider-revamp`

### Plan
- Add a single resolver from provider/model metadata → transport/API kind + providerOptions namespace key.
- Refactor `factory.ts`, `provider-options.ts`, and `reasoning/profile.ts` to use the resolver while preserving current edge-case behavior.
- Add regression tests that lock current behavior by transport + representative model families.
- Continue with gateway ingestion (**1.5.2** OpenRouter, **1.5.3** Vercel AI Gateway) after transport refactor lands cleanly.
...
2026-02-25:
- Confirmed `LLMProvider` is now generated from models.dev + Dexto overlay (`packages/core/src/llm/providers.generated.ts` → `packages/core/src/llm/types.ts`).
- Reasoning PR merged to `main`; canonical provider IDs and reasoning/profile wiring are reconciled in this branch.
- Main remaining architecture gap: reasoning/runtime are still mostly provider-ID branched; next refactor is transport/API-kind keyed routing with explicit exception tables.
...
|------|----------|-----------|
| 2026-02-20 | Auth profiles use deterministic slots (`${providerId}:${methodId}`), no custom user profile names in v1. | Keeps UX simple and avoids naming churn while still allowing multi-method switching without re-auth. |
| 2026-02-25 | Provider identity is models.dev-driven and backward compatibility for legacy IDs/auth is intentionally dropped. | Reduces long-term complexity; no migration layer needed for deprecated local IDs or legacy auth file shape. |
...
| 1.5.1 | Provider snapshot + generated `LLMProvider` | 2026-02-25 | `sync-llm-models` now generates `providers.generated.ts`; `LLM_PROVIDERS`/`LLMProvider` consume generated snapshot (+ Dexto overlay). |
...
| Phase 1.5 — Model/provider registry sources | Partial | 1.5.1 completed (provider snapshot + generated provider union). 1.5.2/1.5.3 gateway ingestion still pending. |
