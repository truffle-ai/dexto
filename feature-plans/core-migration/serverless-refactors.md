...
// TODO: added by human
### open questions
1. How do things like Convex, Neon DB, Supabase, and potentially CloudFlare Durable Objects factor into this? These are very popular tools. I am considering using CloudFlare along with durable objects for our cloud deployment on our proprietary platform, so I want to kind of understand more about those things. I am very new to these technologies.
2. How does this impact our entire bundling logic that we have in ../project-based-architecture.md? The idea here is to allow users to define their configs locally and based on where they want to deploy. Maybe they could provide a deployment config or something. We could even convert local files into remote files, upload them somewhere, and push them as part of bundling. But we will need to figure out how we would handle custom code-based stuff as well like plugins and other places that provide custom code. Would we need custom deployers with deployer config that handle deployment logic after bundling? (based on the deployment config specified). How could infra-as-code sollution work here too? like vercel (accept github repo, vercel auto deploys everything on next.js projects)
