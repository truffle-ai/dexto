...
- models.dev does **not** currently expose which *reasoning effort levels* a model supports
  (e.g. whether OpenAI supports `xhigh` for a specific model). We need a Dexto-owned layer to decide
  which **presets** to expose per provider/model.
...
### 3.1 Current Dexto registry structure (this repo)

Dexto already has a models.dev-backed registry, but it is currently focused on:
- model IDs
- context/input limits
- file upload modality support
- pricing metadata

Key files:
- Registry entry + lookup helpers:
  - `packages/core/src/llm/registry/index.ts`
- Generated snapshot (from models.dev):
  - `packages/core/src/llm/registry/models.generated.ts`
  - generated by `scripts/sync-llm-registry.ts`
- Manual overlays for missing/legacy models:
  - `packages/core/src/llm/registry/models.manual.ts`
- Curated IDs for onboarding/UI pickers:
  - `packages/core/src/llm/curation-config.ts`
- Runtime auto-update (Node-only cache):
  - `packages/core/src/llm/registry/auto-update.ts`
  - `packages/core/src/llm/registry/sync.ts`

Registry caveat relevant to reasoning:
- There is currently a hardcoded heuristic for “OpenAI reasoning-effort capable” models:
  - `packages/core/src/llm/registry/index.ts:1084-1116`
  - This should be replaced by registry/capabilities + preset metadata.

Gateway nuance:
- `openrouter` and `dexto` are marked `supportsAllRegistryModels` in the registry
  (`packages/core/src/llm/registry/index.ts`) but their *transport/provider SDK choice* determines which
  reasoning controls are actually expressible (see Section 4.4).

### 3.2 Proposed: Reasoning preset catalog (opencode-style, but Dexto-owned)

To follow opencode’s paradigm (simple cycling presets, provider-specific under the hood) without overwhelming users:

1) Define a small **portable preset vocabulary** (used when applicable, but never guaranteed for all models):
   - `auto`, `off`, `low`, `medium`, `high`, `max`

2) Compute the **supported preset list per provider+model** (source of truth for UI + config validation):
   - `getSupportedReasoningPresets(provider, model)` returns an ordered list that may be a subset of the
     portable vocabulary, plus optional extras.
   - The UI cycles through exactly this list (e.g. `ctrl+t`), so we never present invalid options.

3) Allow a provider/model to expose additional presets *only when we are confident they’re valid*:
   - OpenAI examples: `minimal`, `xhigh`, `none`
   - This cannot be derived from models.dev today; it must come from Dexto-owned logic/overlays.

4) Implement preset support as a first-class registry concept, so we can “control what’s supported per provider”:
   - `resolveReasoningPreset(provider, model, preset, budgetTokens?)` -> providerOptions payload + warnings

5) Populate the preset catalog using:
   - baseline per-provider defaults (safe, small)
   - plus **explicit per-model overrides** in a Dexto-owned overlay file (similar to `models.manual.ts`), e.g.:
     - add `xhigh` for `openai/gpt-5.2-codex`
     - add `max` budgets for specific Anthropic/Gemini models

This keeps UX simple while still allowing Dexto to be accurate and evolve as providers change.

Cross-reference (Codex shows why we need per-model effort support lists):
- Codex ships a curated list of supported reasoning levels for `gpt-5.2-codex` including `xhigh`:
  - `~/Projects/external/codex/codex-rs/core/models.json`
...
- Add a Dexto-owned reasoning preset catalog (Section 3.2):
  - baseline per-provider presets
