...
export class Telemetry {
    ...
    traceMethod<TMethod extends Function>(
        ...
        return ((...args: unknown[]) => {
            ...
            /**
             * Records the provided error on the current span, sets the span status to ERROR, ends the span, and rethrows the error.
             *
             * @param error - The exception to record and rethrow.
             * @throws The same `error` after it has been recorded on the span.
             */
            ...
            try {
                ...
                /**
                 * Records a method/result value as a span attribute, ends the span, and returns the value.
                 *
                 * Attempts to serialize `res` with `JSON.stringify` and stores it under the attribute
                 * named `${context.spanName}.result`. If serialization fails, stores the literal
                 * string "[Not Serializable]". Always ends the associated span before returning.
                 *
                 * @param res - The value to record on the span; will be returned unmodified
                 * @returns The same value passed in via `res`
                 */
