...
/**
 * Normalize a URL or path string to a lowercase, trimmed path without a trailing slash.
 *
 * Attempts to parse the input as a full URL; if parsing succeeds, returns the URL's
 * pathname lowercased, trimmed, and with any trailing slash removed. If parsing fails,
 * treats the input as a raw path and applies the same normalization.
 *
 * @param url - The URL or path string to normalize.
 * @returns The normalized path (lowercase, trimmed, no trailing slash).
